...
object Crypto {
    ...
    fun seal(roomKey: ByteArray, recipientPk: ByteArray): ByteArray {
        ...
        sodium.cryptoBoxSeal(sealed, roomKey, roomKey.size, recipientPk)
        return sealed
    }

    fun openSeal(sealed: ByteArray, myPk: ByteArray, mySk: ByteArray): ByteArray {
        val opened = ByteArray(sealed.size - Box.SEALBYTES)
        check(sodium.cryptoBoxSealOpen(opened, sealed, sealed.size, myPk, mySk)) { "open seal failed" }
        return opened
    }
    ...
    fun encrypt(roomKey: ByteArray, plaintext: ByteArray, nonce24: ByteArray): ByteArray {
        ...
        sodium.cryptoAeadXChaCha20Poly1305IetfEncrypt(out, null, plaintext, plaintext.size, null, 0, null, nonce24, roomKey)
        return out
    }
    ...
    fun decrypt(roomKey: ByteArray, ciphertext: ByteArray, nonce24: ByteArray): ByteArray {
        ...
        check(sodium.cryptoAeadXChaCha20Poly1305IetfDecrypt(out, null, null, ciphertext, ciphertext.size, null, 0, nonce24, roomKey)) { "decrypt failed" }
        return out
    }

    fun b64(b: ByteArray) = Base64.encodeToString(b, Base64.NO_WRAP)
    fun unb64(s: String) = Base64.decode(s, Base64.DEFAULT)
}
